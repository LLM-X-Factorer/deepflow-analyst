...
3. 计算 **Execution Accuracy**；低于 `EVAL_THRESHOLD`（CI 默认 0.55，当前真实基线 12/20 = 60%）则 fail
4. 上传 `evaluation_report.md` 为 artifact，并写入 PR 的 Step Summary

阈值随基线自然抬升：每次架构改进（CrewAI / LangGraph / RAG）把 accuracy 抬上去后，同步上调 `EVAL_THRESHOLD`，防止回归。
